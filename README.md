...
### Сервер логирования на Go

Для сохранения логов аналитики (linkId, город, страна и другие данные) и предоставления доступа к ним через `/logs` реализован простой сервер на Go.
...
- `POST /api/log` - приём лога в формате JSON
- `GET /logs` - получение всех сохранённых логов

#### Хранение

Логи сохраняются в файл `logs.json` (каждая запись на новой строке).

#### Интеграция с клиентом

Клиент автоматически отправляет данные аналитики на сервер логирования при каждом посещении ссылки с `linkId`. Эндпоинт настраивается через переменную окружения `VITE_LOG_ENDPOINT` в `.env.local`.

#### Запуск вместе с dev сервером
...
Для параллельного запуска сервера и dev-сервера можно использовать два терминала.
...
Полный технический анализ архитектуры, зависимостей и инфраструктуры проекта доступен в файле [ANALYSIS.md](ANALYSIS.md).

## Решение проблемы 500 ошибки

### Проблема
Сервер возвращал ошибку 500 "Failed to save log" при попытке сохранить данные аналитики.

### Причина
Go сервер запущен от пользователя `www-data`, но файл `logs.json` принадлежал `root`. У `www-data` не было прав на запись в файл.

### Решение
1. Изменить владельца файла логов:
   ```bash
   chown www-data:www-data /var/maf-guides/server/logs.json
   ```

2. Обновить конфигурацию systemd сервиса для явного указания пользователя.

3. Проверить работу:
   ```bash
   curl -X POST -H "Content-Type: application/json" \
     -d '{"linkId":"test"}' http://localhost:3002/api/log -v
   ```

Подробное решение в файле [FIXED_SOLUTION.md](FIXED_SOLUTION.md).